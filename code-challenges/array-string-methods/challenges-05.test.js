...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(person =>   {
    const $swSection = $(`<section>${template}</section>`);
    $swSection.find('h2').text(person.name);
    $swSection.find('h3').text(person.height);
    $swSection.find('p').text(person.eye_color);
    $('main').append($swSection)
