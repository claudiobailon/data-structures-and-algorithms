...
const mapCurrentEvents = () => {
  const currentEventsArray = currentEvents.news.map(event => {
...
const returnNames = (arr) => {
  const characterNamesArray = arr.reduce((charName, value) => {
    charName.push(value.name);
    return charName;
  }, []);
  return characterNamesArray;
};
...
const reversedString = (str) => {
  ...
  const reversedStringArr = strArray.reduce((strSoFar, value) => {
    strSoFar = value + strSoFar;
    return strSoFar
  }, '')
  return reversedStringArr; 
};
