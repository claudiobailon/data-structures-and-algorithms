...
const createServer = () => {
  ...
  app.get('/favoritefoods', handleFaveFoods);
  app.get('/*', handleInvalid);

  function handleHello(request,response){
    response.status(200).send('Hello, thanks for stopping by!');
  }
  function handleAboutMe(request,response){
    const bio = `My name is Claudio and I'm a software developer. I write code and have fun doing it!`;
    response.status(200).send(bio);
  }
  function handleFaveFoods(request,response){
    const faveFoods = ['Spaghetti', 'Burritos','Chicken Fried Rice']
    response.status(200).send(faveFoods);
  }
  function handleInvalid(request,response){
    response.status(404).send(`We're sorry, this page does not exist!`);
  }
...
const totalSum = (input) => {
  const oneArray = input.reduce((newArray, currentIndex) => newArray.concat(currentIndex), [])
  const sumOfArray = oneArray.reduce((totalSum, currentNum) => totalSum += currentNum, 0)
  return sumOfArray;
};
